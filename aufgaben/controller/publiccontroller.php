	...
	public function index($token) {
		...
		if ($token) {
			...
			if (is_array($linkItem) && isset($linkItem['uid_owner'])) {
				...
				$itemSource = \OCA\Calendar\App::validateItemSource($linkItem['item_source'],'todo-');
				...
				\OCP\Util::addStyle('aufgaben', 'share');
				\OCP\Util::addScript('aufgaben', 'share');
				
				$data = \OCA\Aufgaben\App::getEventObject($itemSource, false, false);
				$l = \OC::$server->getL10N('aufgaben');
				$object = \OCA\Calendar\VObject::parse($data['calendardata']);
				$vTodo = $object -> VTODO;
				$id=$data['id'];
				...
				//PERCENT-COMPLETE
			$percentComplete=0;
			$cptlStatus='Nicht begonnen';	
			if($vTodo->{'PERCENT-COMPLETE'}){
				$percentComplete =  $vTodo->{'PERCENT-COMPLETE'};
				$cptlStatus='In Arbeit';
				if($vTodo->{'PERCENT-COMPLETE'} == '0'){
					$cptlStatus='Nicht begonnen';	
				}
			}
		...
		$response = new TemplateResponse('aufgaben', 'publicevent',$params,'base');
