...
Timeline::$l10n = \OC::$server->getL10N('aufgaben');
class Timeline{
	...
	public function setTimeLineDay($sDay){
		$this->sDaySelect=$sDay;
		 \OCP\Util::writeLog('aufgaben','SETTER TIME: '.date('d.m.Y',$this->sDaySelect), \OCP\Util::DEBUG);
	}
	...
	public function generateAddonCalendarTodo(){
		...
		// foreach( $this->aCalendar as $cal ) {
			 ...
			 foreach( $calendar_tasks as $taskInfo ) {
					...
					$object = \OCA\Calendar\VObject::parse($taskInfo['calendardata']);
					$vtodo = $object->VTODO; 
			 ...
			 $aTaskTime['actweek']=$this->getStartofTheWeek().' - '.$this->getEndofTheWeek();
				...
				//foreach( $this->aCalendar as $cal ) {
				  ...
				  foreach( $calendar_tasks as $task ) {
					   ...
					   $object = \OCA\Calendar\VObject::parse($task['calendardata']);
					   $vtodo = $object->VTODO;
					   $due = $vtodo->DUE;
			 ...
			 $aExec=array();
   	         ...
   	         switch($this->sMode){
			 	case 'dayselect':
					 ...
					 \OCP\Util::writeLog('aufgaben','SDAY BEGINNTIME: '.$start, \OCP\Util::DEBUG);
