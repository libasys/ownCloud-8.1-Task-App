...
class Provider extends \OCP\Search\Provider {
	...
	function search($query) {
		...
		//foreach($calendars as $calendar) {
			$objects = \OCA\Aufgaben\App::all($activeCalendars);
			foreach($objects as $object) {
				...
				if(substr_count(strtolower($object['summary']), strtolower($query)) > 0 || $searchAdvanced==true) {
					...
					$link = \OC::$server->getURLGenerator()->linkToRoute('aufgaben.page.index').'#'.urlencode($object['id']);
					$returnData['id']=$object['id'];
					$returnData['description']=$object['summary'].' '.$info;
					$returnData['link']=$link;
					
					$results[]=new \OCA\Aufgaben\Search\Result($returnData);
